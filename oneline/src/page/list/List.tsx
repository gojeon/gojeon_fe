...
function List() {
    ...
    const [param, setParam] = useState({ text: '', writer: '' });
    ...
    const onChangeParam = (e: { target: { value: any } }, key: string) => {
        setParam((prev) => ({ ...prev, [key]: e.target.value }));
    };
        ...
        <MantineProvider>
            <div css={backgroundWrapperStyle}>
                <Center style={{ minHeight: '100vh' }}>
                    <h1 style={{ textAlign: 'center' }}>문장 한 숟갈</h1>
                    {data.length > 0 &&
                        data.map((item, idx) => (
                            <div key={item.id}>
                                {editing.id === item.id ? (
                                    <>
                                        ...
                                        <button onClick={onClickUpdate}>문장 수정</button>
                                    </>
                                ) : (
                                    <>
                                        <span>{idx + 1}</span>
                ...
                <div>
                    ...
                    <textarea
                        onChange={(e) => {
                            onChangeParam(e, 'text');
                        }}
                        value={param.text}
                    />
                    <button onClick={onClickWrite}>문장 제출하기</button>
                </div>
            </div>
        </MantineProvider>
    );
